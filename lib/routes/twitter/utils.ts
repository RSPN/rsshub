...
// throw new Error("UTILS.TS LOADED");

const getQueryParams = (url) => URL.parse(url, true).query;
const getOriginalImg = (url) => {
    // Always return the direct image file URL, e.g., https://pbs.twimg.com/media/Gu4jm52XQAApzu2.jpg
    let m = null;
    // Case 1: Already a direct file link
    if ((m = url.match(/^(https?:\/\/\w+\.twimg\.com\/media\/[^/:]+)\.(jpg|jpeg|gif|png|bmp|webp)(:\w+)?/i))) {
        let format = m[2];
        if (format === 'jpeg') format = 'jpg';
        return `${m[1]}.${format}`;
    }
    // Case 2: Has ?format=xxx in any position in the query string
    if ((m = url.match(/^(https?:\/\/\w+\.twimg\.com\/media\/[^/:]+)\?(.*)$/i))) {
        const base = m[1];
        const query = m[2];
        // Match format=xxx anywhere in the query string
        const formatMatch = query.match(/(?:^|[&?])format=([a-z]+)/i);
        if (formatMatch) {
